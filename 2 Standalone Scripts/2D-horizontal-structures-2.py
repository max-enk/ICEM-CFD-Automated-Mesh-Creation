...
meshtype = "2D-horizontal-1"    # 2D mesh with horizontal structures
...
print("#################################################################### WELCOME ####################################################################\n")
print("This is a script for the automatic generation of a 2D mesh with horizontal structures in ICEM. Features:\n    - additional gas space above film inlet\n    - shared outlet for gas and liquid phases")
        ...
        while True:
            ...
            if os.path.exists(refconffile):
                ...
                if dimmode == True:
                    ...
                    print("The config file specified is not suited for 2D mesh creation with horizontal structures.")
            else:
...
pnts.append(Point(h_t.getval(), (l_t.getval() - l_i.getval()), 0.0))
pnts.append(Point(0.0, (l_t.getval() - l_i.getval()), 0.0))
pnts.append(Point(-1*d_gr.getval(), (l_t.getval() - l_i.getval()), 0.0))

# curves with part association (boundaries)
crvs.append(Curve(pnts[8], pnts[7]))
geom_fwall.append(crvs[-1])
crvs.append(Curve(pnts[7], pnts[0]))
geom_fwall.append(crvs[-1])
...
for i in range(ns):
    # first row of points
    for j in range(3):
        pnts.append(Point(pnts[-3].getx(), (pnts[-3].gety() - l_gr.getval()), 0.0))
    # second row of points
    for j in range(3):
        pnts.append(Point(pnts[-3].getx(), (pnts[-3].gety() - l_s.getval()), 0.0))   
    # curves with part association (boundaries)
    crvs.append(Curve(pnts[-1], pnts[-2]))
    geom_fwall.append(crvs[-1])
    crvs.append(Curve(pnts[-2], pnts[-5]))
    geom_fwall.append(crvs[-1])
    crvs.append(Curve(pnts[-5], pnts[-4]))
    geom_fwall.append(crvs[-1])
    crvs.append(Curve(pnts[-4], pnts[-7]))
    geom_fwall.append(crvs[-1])
    crvs.append(Curve(pnts[-3], pnts[-6]))
    geom_gwall.append(crvs[-1])
    crvs.append(Curve(pnts[-6], pnts[-9]))
    geom_gwall.append(crvs[-1])
...
# curves with part association (boundaries)
crvs.append(Curve(pnts[-10], pnts[-7]))
geom_gwall.append(crvs[-1])
crvs.append(Curve(pnts[-7], pnts[-4]))
geom_gwall.append(crvs[-1])
...
geom_fwall.append(crvs[-1])
crvs.append(Curve(pnts[-5], pnts[-8]))
geom_fwall.append(crvs[-1])
print(" - done bottom\n")
...
blkg.append(Split(pnts[0], 37, 38))
blkg.append(Split(pnts[1], 43, 38))
blkg.append(Split(pnts[8], 11, 37))
print(" - done top")

# structures
for splt in range(2*ns + 1):
    blkg.append(Split(pnts[11 + 3*splt], 11, 52 + 7*splt))
for splt in range(ns + 2):
    blkg.append(Delete(25 + 12*splt))
print(" - done structures")

# bottom section at outlet
blkg.append(Delete(4))
print(" - done bottom\n")    

# edge-curves associations
print("Associating edges to curves...")

# top section at inlet
edges.append(Edge(52, 53, crvs[0]))
edges.append(Edge(53, 43, crvs[1]))
edges.append(Edge(43, 48, crvs[2]))
edges.append(Edge(48, 38, crvs[3]))
edges.append(Edge(38, 34, crvs[4]))
edges.append(Edge(34, 21, crvs[5]))
edges.append(Edge(39, 21, crvs[6]))
edges.append(Edge(56, 39, crvs[7]))
print(" - done top")

# structures
for crv in range(ns):
    edges.append(Edge(66 + crv*14, 67 + crv*14, crvs[edges[-1].getcrv().getnum() + 1]))
    edges.append(Edge(67 + crv*14, 60 + crv*14, crvs[edges[-1].getcrv().getnum() + 1]))
    edges.append(Edge(59 + crv*14, 60 + crv*14, crvs[edges[-1].getcrv().getnum() + 1]))
    edges.append(Edge(59 + crv*14, 52 + crv*14, crvs[edges[-1].getcrv().getnum() + 1]))
    edges.append(Edge(70 + crv*14, 63 + crv*14, crvs[edges[-1].getcrv().getnum() + 1]))
    edges.append(Edge(63 + crv*14, 56 + crv*14, crvs[edges[-1].getcrv().getnum() + 1]))
print(" - done structures")

# bottom section at outlet
edges.append(Edge(77 + (ns-1)*14 , 70 + (ns-1)*14, crvs[-8]))
edges.append(Edge(19, 77 + (ns-1)*14, crvs[-7]))
edges.append(Edge(33, 19, crvs[-6]))
edges.append(Edge(47, 33, crvs[-5]))
edges.append(Edge(42, 47, crvs[-4]))
edges.append(Edge(74 + (ns-1)*14, 42, crvs[-3]))
edges.append(Edge(73 + (ns-1)*14, 74 + (ns-1)*14, crvs[-2]))
edges.append(Edge(66 + (ns-1)*14, 73 + (ns-1)*14, crvs[-1]))
print(" - done bottom\n")
...
# x direction
mshg.append(Mesh(52, 53, d_gr.getmesh()))   # grooves
mshg.append(Mesh(43, 48, h_nu.getmesh()))   # film
mshg.append(Mesh(48, 38, h_d.getmesh()))    # distributor
mshg.append(Mesh(38, 39, h_g.getmesh()))    # gas space
print(" - done x direction")

# y direction
# top section at inlet
mshg.append(Mesh(38, 34, l_g.getmesh()))    # gas space
mshg.append(Mesh(53, 43, l_i.getmesh()))    # smooth inlet
# structures
for edge in range(ns):
    mshg.append(Mesh(mshg[-1].getvert1() + 6, mshg[-1].getvert1() - 1, l_gr.getmesh()))     # groove
    mshg.append(Mesh(mshg[-1].getvert1() + 8, mshg[-1].getvert1() + 1, l_s.getmesh()))      # structure
# bottom section at outlet
mshg.append(Mesh(mshg[-1].getvert1() + 6, mshg[-1].getvert1() - 1, l_gr.getmesh()))         # last groove
mshg.append(Mesh(42, mshg[-1].getvert1() + 1, l_o.getmesh()))                               # smooth outlet
